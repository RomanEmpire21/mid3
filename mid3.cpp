// Jonathan Roman
// Framework was given by Gordon. Unit
// test was done by Jonathan.
// 
...
#ifdef UTEST
void U_Test(double *values, double tolerance);
#endif
...
void U_Test(double *values, double tolerance)
{
    // Values contains 479 values
    // 160 rows are in values
    int idx = 0;
    int idx2 = 0;
    int errors = 0;
    double ponderal[200];
    ...
    cout << "--------------------------------------------\n";
    for (int i=0; i<160; i++) {
        ponderal[i] = getPonderal(values[idx2], values[idx2+1]);
        idx2 += 3;
    }
    for (int i=0; i<160; i++) {
